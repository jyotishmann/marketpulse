...
def test_create_scheduler_has_four_jobs():
    ...
    assert {"warmup_cache", "ingest_stock_data", "ingest_news", "run_ml_pipeline"} == job_ids
...
def test_ingest_news_skips_when_no_urls():
    with patch("marketpulse.scheduler.jobs.settings") as mock_settings, \
...
def test_warmup_cache_skips_when_db_unavailable(monkeypatch):
    with patch("marketpulse.scheduler.jobs.verify_connection", return_value=False), \
